...
function getAllPosts() {
  try {
    fetch(`${link}/post/`,{
      ...
      .then((data) => {
        // unselected_class = "outline btn-floating white waves-effect z-depth-0 right"
        // unselected_icon = "light-blue-text material-icons"
        console.log(data);
        output = `<div></div>`;
        data.forEach((post) => {
          if(post.created_by != sessionStorage.currentUser){
            years = "";
            tech_st = ``;
